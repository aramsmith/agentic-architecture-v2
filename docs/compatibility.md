...
| Human approval | `extensions.approvalMode` declares `synthetic`, `self-asserted`, or `human-verified`; the declaration must match the evidence | **Verified for record integrity, not for identity** | An approval proves reviewer convergence on identical hashes and internal consistency. It does **not** prove who decided, or that a human decided. Signature verification is not yet implemented, so every real approval is `self-asserted`. |
